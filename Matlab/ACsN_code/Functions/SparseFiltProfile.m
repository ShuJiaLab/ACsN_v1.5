% SparseFiltProfile: Specify parameters for Sparse filtering
classdef SparseFiltProfile
    properties (Constant)
        NO_VALUE = -1;
        HARD_THRESHOLDING = int8(1);
        WIENER_FILTERING = int8(2);
        ALL_STAGES = SparseFiltProfile.HARD_THRESHOLDING + SparseFiltProfile.WIENER_FILTERING;
    end
    properties
        ...
        Nf = 3;  % domain size for FFT computations
        Kin = 1;  % how many layers of var3D to calculate
        ...
        N2                  = 8;  %% maximum number of similar blocks (maximum size of the 3rd dimension of a 3D array)
        Ns                  = 19;  %% length of the side of the search neighborhood for full-search block-matching (BM), must be odd
        tau_match           = 2500;%% threshold for the block-distance (d-distance)
    ...
    methods
        ...
        function pro = SparseFiltProfile(profileName)
