...
class BM3DProfile:
    ...
    # Variance calculation parameters //SUBJECT TO CHANGE
    nf = 0  # domain size for FFT computations --> changed from 32
    k = 0  # how many layers of var3D to calculate --> changed from 1 #doesn't seem to affect score

    # Refiltering //SUBJECT TO CHANGE
    denoise_residual = False  # Perform residual thresholding and re-denoising
    residual_thr = 0  # Threshold for the residual HT (times sqrt(PSD))
    max_pad_size = [-1, -1]  # Maximum required pad size (= half of the kernel size), or 0 -> use image size // changed from None

    # Block matching
    gamma = 1.0  # Block matching correction factor // changed from 4.0
    ...
    max_3d_size_ht = 8  # maximum number of similar blocks (maximum size of the 3rd dimension of a 3D array) // changed from 16
    search_window_ht = 19  # side length of the search neighborhood for full-search block-matching (BM), must be odd // changed from 39
    tau_match = 2500  # threshold for the block-distance (d-distance) // changed from 3000

    # None in these parameters results in automatic parameter selection for them
    lambda_thr3d = 0.01  # 2.7  # threshold parameter for the hard-thresholding in 3D transform domain //changed from None
    mu2 = None  # 1.0
    # Refilter
    lambda_thr3d_re = 0.01 # changed from None
    mu2_re = None
    ...
    bs_wiener = 8
    step_wiener = 3
    max_3d_size_wiener = 16  #changed from 32
    search_window_wiener = 19  #changed from 39
